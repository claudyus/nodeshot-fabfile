...
def initialize_ssl(use_defaults=False):
    with quiet():
        openssl_installed = run_wrapper('which openssl').succeeded

    if not openssl_installed:
        print(green("openssl command not found, installing it..."))
        with hide('everything'):
            run_wrapper('apt-get install -y openssl')
    ...
    with cd(install_dir):
        run_wrapper(openssl_command)


def _set_log_permissions():
    run_wrapper('chown -R {user}:www-data {path}/log'.format(user=env['user'], path=nodeshot_dir))
    run_wrapper('chmod -R 775 {path}/log'.format(path=nodeshot_dir))
...
def create_install_dir():
    ...
    with hide('everything'):
        run_wrapper('mkdir -p %s' % install_dir)
    with cd(install_dir), hide('everything'):
        install_dir = run_wrapper('pwd')
...
def get_os_version():
    ...
    with quiet():
        version_file = run_wrapper('cat /etc/issue')
...
def install_dependencies():
    ...
    with hide('everything'):
        ...
        # install
        run_wrapper('apt-get install -y %s' % dependencies)
        # install Postgis 2
        with quiet():
            postgis_installed = run_wrapper('dpkg --get-selections | grep "postgis\s"').succeeded
        if not postgis_installed:
            with cd(install_dir):
                run_wrapper('wget http://download.osgeo.org/postgis/source/postgis-2.1.3.tar.gz')
                run_wrapper('tar xfvz postgis-2.1.3.tar.gz')
            with cd('%s/postgis-2.1.3' % install_dir):
                run_wrapper('./configure')
                run_wrapper('make')
            # on debian 7 the procedure aborts if we don't do this
            with quiet():
                if not exists('/usr/share/postgresql/9.1/contrib', use_sudo=use_sudo):
                    run_wrapper("mkdir -p '/usr/share/postgresql/9.1/contrib/postgis-2.1'")
            with cd('%s/postgis-2.1.3' % install_dir):
                run_wrapper('checkinstall -y')
...
def create_db():
    ...
    with hide('everything'):
        ...
        run_wrapper('chmod 777 /tmp/db.sql')
        run_wrapper('su - postgres -c "psql -f /tmp/db.sql"')
        run_wrapper('rm /tmp/db.sql')
...
def create_python_virtualenv():
    ...
    with hide('everything'):
        run_wrapper('pip install virtualenvwrapper')
        run_wrapper("echo 'export WORKON_HOME={workon_home}' >> /usr/local/bin/virtualenvwrapper.sh".format(workon_home=workon_home))
        run_wrapper("echo 'source /usr/local/bin/virtualenvwrapper.sh' >> ~/.bashrc")
        run_wrapper("echo 'source /usr/local/bin/virtualenvwrapper.sh' >> ~/.bash_profile")
        run_wrapper('mkdir -p {0}'.format(workon_home))
        run_wrapper("chown -R {user}:{user} {workon_home}".format(user=env['user'], workon_home=workon_home))
        run_wrapper('chmod -R 775 {workon_home}'.format(workon_home=workon_home))
        run_wrapper('mkvirtualenv nodeshot')
...
def install_python_requirements():
    ...
    with hide('everything'):
        run_wrapper('workon nodeshot && pip install -U pip wheel setuptools')
        run_wrapper('workon nodeshot && pip install -U https://github.com/ninuxorg/nodeshot/tarball/master')
...
def create_project():
    ...
    with hide('everything'):
        run_wrapper('mkdir -p %s' % nodeshot_dir)
    with cd(root_dir), hide('everything'):
        run_wrapper('workon nodeshot && nodeshot startproject %s nodeshot' % project_name)
    print(green("Setting permissions..."))
    with cd(nodeshot_dir), hide('everything'):
        run_wrapper('chown -R %s:www-data .' % env['user'])
        run_wrapper('adduser www-data %s' % env['user'])
        run_wrapper('chmod 775 . log {name} {name}/media'.format(name=project_name))
        run_wrapper('chmod 750 manage.py ./%s/*.py' % project_name)
...
def edit_settings():
    ...
    with cd('%s/%s' % (nodeshot_dir, project_name)), hide('everything'):
        run_wrapper('sed -i \'s#<user>#%s#g\' settings.py' % db_user)
        run_wrapper('sed -i \'s#<password>#%s#g\' settings.py' % db_pass)
        run_wrapper('sed -i \'s#<domain>#%s#g\' settings.py' % server_name)
        run_wrapper('sed -i \'s#DEBUG = True#DEBUG = False#g\' settings.py')
...
def install_redis():
    ...
    with hide('everything'):
        run_wrapper('apt-get -y --force-yes install redis-server')
        run_wrapper('workon nodeshot && pip install -U celery[redis]')
        with warn_only():
            run_wrapper('echo 1 > /proc/sys/vm/overcommit_memory')
        run_wrapper('service redis-server start')
        sleep(5)
...
def sync_data(update=None):
    ...
    with cd(nodeshot_dir), hide('everything'):
        ...
        run_wrapper('workon nodeshot && %s' % sync_command)
        # only on install
        if not update:
            run_wrapper('workon nodeshot && ./manage.py loaddata initial_data')
...
def create_admin():
    ...
    with cd(nodeshot_dir), hide('everything'):
        run_wrapper('workon nodeshot && %s' % create_admin_oneliner)
...
def configure_nginx():
    ...
    with hide('everything'):
        run_wrapper('mkdir -p %s' % nginx_ssl_dir)
    with cd(nginx_ssl_dir), hide('everything'):
        run_wrapper('cp ~/nodeshot_install/server.crt .')
        run_wrapper('cp ~/nodeshot_install/server.key .')
        run_wrapper('cp /etc/nginx/uwsgi_params /etc/nginx/sites-available/')
        run_wrapper('mkdir -p %s/public_html' % nodeshot_dir)
    ...
    with cd('/etc/nginx/sites-available'), hide('everything'):
        run_wrapper('ln -s /etc/nginx/sites-available/{0} /etc/nginx/sites-enabled/{0}'.format(server_name))
        run_wrapper('service nginx configtest')
...
def configure_supervisor():
    ...
    with hide('everything'):
        ...
        _set_log_permissions()
        run_wrapper('supervisorctl update')
...
def install_postfix():
    ...
    with hide('everything'):
        run_wrapper('export DEBIAN_FRONTEND=noninteractive && apt-get -y install postfix')
...
def restart_services():
    ...
    with hide('everything'):
        run_wrapper('service nginx restart')
    print(green("Restarting processes..."))
    with hide('everything'):
        _set_log_permissions()
        run_wrapper('supervisorctl restart all')


def remove_install_dir():
    print(green("Removing install dir..."))
    with hide('everything'):
        run_wrapper('rm -rf ~/nodeshot_install')
    ...
    with hide('everything'):
        while True:
            ...
            for process in processes.keys():
                output = run_wrapper('supervisorctl status {0}'.format(process))
